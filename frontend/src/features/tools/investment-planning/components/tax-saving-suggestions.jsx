...
const TaxSavingSuggestions = ({
  ...
  return (
    <div className="space-y-6">
      {/* Tax Savings Summary */}
      {taxSavings && (
        <div className="bg-gradient-to-r from-gold-50 to-gold-100 border border-gold-200 rounded-xl p-6">
          <h3 className="font-semibold text-gold-900 mb-4">Potential Tax Savings</h3>
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            <div className="bg-white rounded-xl p-4">
              <p className="text-body-regular text-slate-600">Total Deduction</p>
              <p className="text-heading-2 font-bold text-gold-600">
                ₹{taxSavings.totalDeduction?.toLocaleString('en-IN')}
              </p>
            </div>
            <div className="bg-white rounded-xl p-4">
              <p className="text-body-regular text-slate-600">Tax Savings</p>
              <p className="text-heading-2 font-bold text-green-600">
                ₹{taxSavings.totalSavings?.toLocaleString('en-IN')}
              </p>
            </div>
            <div className="bg-white rounded-xl p-4">
              <p className="text-body-regular text-slate-600">Effective Tax Rate</p>
              <p className="text-heading-2 font-bold text-blue-600">{taxSavings.effectiveTaxRate}%</p>
            </div>
          </div>
        </div>
      )}
      ...
      <div className="space-y-4">
        ...
        {recommendations.map((rec, index) => (
          <div
            key={index}
            className="border border-slate-200 rounded-xl p-4 hover:border-gold-300 transition-colors"
          >
            <div className="flex items-start justify-between">
              <div className="flex-1">
                <h4 className="font-semibold text-slate-900 mb-1">{rec.type}</h4>
                <p className="text-body-regular text-slate-600 mb-2">{rec.description}</p>
                <div className="flex flex-wrap gap-4 text-body-small text-slate-500">
                ...
                {rec.benefits && rec.benefits.length > 0 && (
                  <ul className="mt-2 space-y-1">
                    {rec.benefits.map((benefit, idx) => (
                      <li key={idx} className="text-body-small text-slate-600 flex items-center gap-1">
      ...
      {availableAmount && (
        <div className="bg-blue-50 border border-blue-200 rounded-xl p-4">
          <div className="flex items-center justify-between">
            <span className="text-body-regular text-blue-800">Total Investment Required:</span>
          ...
          {availableAmount < recommendations.reduce((sum, rec) => sum + (rec.investmentAmount || 0), 0) && (
            <p className="text-body-small text-error-600 mt-2">
