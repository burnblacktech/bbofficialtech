    ...
    <div className="space-y-6">
      ...
      <div className="bg-gradient-to-r from-blue-50 to-indigo-50 rounded-xl p-4 border border-blue-200">
        <div className="flex items-start space-x-3">
          ...
          <div className="flex-1">
            <h4 className="text-sm font-semibold text-slate-900">Auto-fill from Form 26AS</h4>
            ...
            {sources?.form26as?.available && (
              <div className="mt-2 flex items-center text-body-small text-green-700">
      ...
      <div className="space-y-4">
        <div className="flex items-center justify-between">
          ...
          <button
            ...
            className="flex items-center text-body-regular text-gold-600 hover:text-gold-700 font-medium"
          >
            ...
            <div className="relative">
              ...
              <input
                ...
                className={`w-full pl-8 pr-4 py-3 border rounded-xl focus:outline-none focus:ring-2 focus:ring-gold-500 text-lg font-semibold ${
                  isAutoFilled('tds') ? 'border-blue-300 bg-blue-50/30' : 'border-slate-300'
                }`}
                placeholder="0"
              />
            </div>
            <p className="text-body-small text-slate-500 mt-1">Total TDS deducted by all deductors</p>
          </div>
        )}
        ...
        {showTDSDetails && (
          <div className="space-y-3">
            {tdsEntries.map((entry, index) => (
              <div key={entry.id} className="bg-slate-50 border border-slate-200 rounded-xl p-4">
                <div className="flex items-center justify-between mb-3">
                  <span className="text-body-regular font-medium text-slate-700">TDS Entry {index + 1}</span>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-5 gap-3">
                  <div className="lg:col-span-2">
                    <label className="block text-body-small font-medium text-slate-600 mb-1">Deductor Name</label>
                    <input
                      type="text"
                      value={entry.deductorName}
                      onChange={(e) => handleTDSEntryChange(entry.id, 'deductorName', e.target.value)}
                      className="w-full px-3 py-2 border border-slate-300 rounded-xl focus:outline-none focus:ring-2 focus:ring-gold-500 text-body-regular"
                      placeholder="Company/Employer Name"
                    />
                  </div>
                  <div>
                    <label className="block text-body-small font-medium text-slate-600 mb-1">TAN</label>
                    <input
                      type="text"
                      value={entry.tan}
                      onChange={(e) => handleTDSEntryChange(entry.id, 'tan', e.target.value.toUpperCase())}
                      maxLength={10}
                      className="w-full px-3 py-2 border border-slate-300 rounded-xl focus:outline-none focus:ring-2 focus:ring-gold-500 text-body-regular font-mono"
                      placeholder="ABCD12345E"
                    />
                  </div>
                  <div>
                    <label className="block text-body-small font-medium text-slate-600 mb-1">Section</label>
                    <select
                      value={entry.section}
                  ...
                  <div>
                    <label className="block text-body-small font-medium text-slate-600 mb-1">Amount (₹)</label>
                    <input
                      type="number"
                      value={entry.amount}
                      onChange={(e) => handleTDSEntryChange(entry.id, 'amount', e.target.value)}
                      className="w-full px-3 py-2 border border-slate-300 rounded-xl focus:outline-none focus:ring-2 focus:ring-gold-500 text-body-regular font-semibold"
                      placeholder="0"
                    />
                  </div>
                </div>
            ...
            <button
              ...
              className="w-full py-3 border-2 border-dashed border-slate-300 rounded-xl text-slate-600 hover:border-gold-400 hover:text-gold-600 transition-colors flex items-center justify-center"
            >
        ...
        <div>
          <div className="relative">
            ...
            <input
              ...
              className="w-full pl-8 pr-4 py-2.5 border border-slate-300 rounded-xl focus:outline-none focus:ring-2 focus:ring-gold-500"
              placeholder="0"
            />
          </div>
          <p className="text-body-small text-slate-500 mt-1">TCS collected (e.g., on sale of goods, foreign remittance)</p>
        </div>
      </div>
      ...
      <div className="space-y-4">
        ...
        {!showAdvanceTaxDetails && (
          <div>
            ...
            <p className="text-body-small text-slate-500 mt-1">Total advance tax paid during the year</p>
          </div>
        )}
        ...
        {showAdvanceTaxDetails && (
          <div className="space-y-3">
            {advanceTaxEntries.map((entry, index) => (
              <div key={entry.id} className="bg-slate-50 border border-slate-200 rounded-xl p-4">
                <div className="flex items-center justify-between mb-3">
                  <span className="text-body-regular font-medium text-slate-700">Challan {index + 1}</span>
                  <button
                    type="button"
                ...
                <div className="grid grid-cols-1 md:grid-cols-4 gap-3">
                  <div>
                    <label className="block text-body-small font-medium text-slate-600 mb-1">BSR Code</label>
                    <input
                      type="text"
                      value={entry.bsrCode}
                  ...
                  <div>
                    <label className="block text-body-small font-medium text-slate-600 mb-1">Date of Deposit</label>
                    <input
                      type="date"
                  ...
                  <div>
                    <label className="block text-body-small font-medium text-slate-600 mb-1">Challan Serial No.</label>
                    <input
                      type="text"
      ...
      <div className="space-y-4">
        ...
        <div>
          ...
          <p className="text-body-small text-slate-500 mt-1">Tax paid after calculating final tax liability (before filing)</p>
        </div>
      </div>
      ...
      <div className="bg-gradient-to-r from-green-50 to-emerald-50 rounded-xl p-4 border border-green-200">
        <div className="flex items-center mb-3">
          ...
          <h4 className="text-sm font-semibold text-slate-900">Taxes Paid Summary</h4>
        </div>

        <div className="space-y-2">
          <div className="flex justify-between items-center text-body-regular">
            <span className="text-slate-600">TDS</span>
            <span className="font-medium text-slate-900">{formatCurrency(totalTDS)}</span>
          </div>
          <div className="flex justify-between items-center text-body-regular">
            <span className="text-slate-600">TCS</span>
            <span className="font-medium text-slate-900">{formatCurrency(data?.tcs || 0)}</span>
          </div>
          <div className="flex justify-between items-center text-body-regular">
            <span className="text-slate-600">Advance Tax</span>
            <span className="font-medium text-slate-900">{formatCurrency(totalAdvanceTax)}</span>
          </div>
          <div className="flex justify-between items-center text-body-regular">
            <span className="text-slate-600">Self Assessment Tax</span>
            <span className="font-medium text-slate-900">{formatCurrency(data?.selfAssessmentTax || 0)}</span>
          </div>
          <div className="pt-2 mt-2 border-t border-green-200">
            <div className="flex justify-between items-center">
              <span className="text-sm font-semibold text-slate-900">Total Taxes Paid</span>
              <span className="text-body-large font-bold text-green-600">{formatCurrency(totalTaxesPaid)}</span>
            </div>
          </div>
        </div>
      </div>
