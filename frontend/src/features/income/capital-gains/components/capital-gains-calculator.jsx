  ...
  return (
    <div className="bg-white rounded-xl border border-slate-200 p-6">
      ...
      <div className="space-y-4">
        <div>
          <label className="block text-body-sm font-medium text-slate-700 mb-2">Asset Type</label>
          <select
            value={formData.assetType}
            onChange={(e) => handleChange('assetType', e.target.value)}
            className={`w-full px-4 py-2 border rounded-xl focus:outline-none focus:ring-2 focus:ring-gold-500 ${
              errors.assetType ? 'border-error-300' : 'border-slate-300'
            }`}
          >
          ...
          {errors.assetType && (
            <p className="text-body-sm text-error-600 mt-1">{errors.assetType}</p>
          )}
        </div>

        <div className="grid grid-cols-2 gap-4">
          <div>
            <label className="block text-body-sm font-medium text-slate-700 mb-2">Purchase Date</label>
            <input
              type="date"
          ...
          <div>
            <label className="block text-body-sm font-medium text-slate-700 mb-2">Sale Date</label>
            <input
              type="date"
              value={formData.saleDate}
              onChange={(e) => handleChange('saleDate', e.target.value)}
              className={`w-full px-4 py-2 border rounded-xl focus:outline-none focus:ring-2 focus:ring-gold-500 ${
                errors.saleDate ? 'border-error-300' : 'border-slate-300'
              }`}
            />
            {errors.saleDate && (
              <p className="text-body-sm text-error-600 mt-1">{errors.saleDate}</p>
            )}
          </div>
        </div>
        ...
        <div>
          <label className="block text-body-sm font-medium text-slate-700 mb-2">Sale Value (₹)</label>
          <input
            type="number"
            value={formData.saleValue}
            onChange={(e) => handleChange('saleValue', e.target.value)}
            className={`w-full px-4 py-2 border rounded-xl focus:outline-none focus:ring-2 focus:ring-gold-500 ${
              errors.saleValue ? 'border-error-300' : 'border-slate-300'
            }`}
            placeholder="Enter sale value"
          />
          {errors.saleValue && (
            <p className="text-body-sm text-error-600 mt-1">{errors.saleValue}</p>
          )}
        </div>
        ...
        {formData.isLongTerm ? (
          ...
          <div>
            <label className="block text-body-sm font-medium text-slate-700 mb-2">Purchase Value (₹)</label>
            <input
              type="number"
              value={formData.purchaseValue}
              onChange={(e) => handleChange('purchaseValue', e.target.value)}
              className={`w-full px-4 py-2 border rounded-xl focus:outline-none focus:ring-2 focus:ring-gold-500 ${
                errors.purchaseValue ? 'border-error-300' : 'border-slate-300'
              }`}
              placeholder="Enter purchase value"
            />
            {errors.purchaseValue && (
              <p className="text-body-sm text-error-600 mt-1">{errors.purchaseValue}</p>
            )}
          </div>
        )}

        <div>
          <label className="block text-body-sm font-medium text-slate-700 mb-2">Expenses on Transfer (₹)</label>
          <input
            type="number"
        ...
        <div className="mt-6 bg-green-50 border border-green-200 rounded-xl p-6">
          ...
          <div className="space-y-3">
            <div className="flex justify-between">
              <span className="text-body-sm text-slate-600">Sale Value:</span>
              <span className="text-body-md font-semibold text-gray-800">
                {formatCurrency(result.saleValue)}
              </span>
            </div>
            <div className="flex justify-between">
              <span className="text-body-sm text-slate-600">Cost of Acquisition:</span>
              <span className="text-body-md font-semibold text-gray-800">
                {formatCurrency(result.costOfAcquisition)}
              </span>
            </div>
            <div className="flex justify-between">
              <span className="text-body-sm text-slate-600">Expenses:</span>
              <span className="text-body-md font-semibold text-gray-800">
