...
const Section80DDB = ({ filingId, onUpdate }) => {
  ...
  const updateDeductionMutation = useMutation({
    mutationFn: async ({ deductionId, data }) => {
      try {
        const response = await apiClient.put(`/api/itr/deductions/80DDB/${deductionId}`, { filingId, ...data });
        return response.data;
      } catch (error) {
        const limit = data.isSeniorCitizen ? 100000 : 40000;
        const amount = Math.min(parseFloat(data.amount) || 0, limit);
    ...
    onSuccess: (result) => {
      ...
      if (typeof onUpdate === 'function') {
        onUpdate({ section80DDB: result?.data?.totalAmount ?? 0 });
      }
    },
    onError: (error) => {
  ...
  const deleteDeductionMutation = useMutation({
    mutationFn: async (deductionId) => {
      try {
        const response = await apiClient.delete(`/api/itr/deductions/80DDB/${deductionId}?filingId=${filingId}`);
        return response.data;
      } catch (error) {
        if (onUpdate) {
    ...
    <div className="space-y-6">
      ...
      <div className="bg-white rounded-xl shadow-elevation-1 border border-slate-200 p-6">
        <div className="flex items-center justify-between mb-4">
          <div className="flex items-center space-x-3">
            ...
            <div>
              <h2 className="text-heading-lg text-slate-900">Section 80DDB</h2>
              <p className="text-body-sm text-slate-600">Medical Treatment (Self/Dependent: ₹40,000 | Senior Citizen: ₹1,00,000)</p>
            </div>
          </div>
          <button
      ...
      {deductions.length > 0 ? (
                  ...
                  <div className="flex items-start space-x-4 flex-1">
                    ...
                    <div className="flex-1">
                      <div className="flex items-center space-x-2 mb-2">
                        <h3 className="text-heading-md text-slate-900">{deduction.patientName}</h3>
                        {deduction.isVerified && (
                          <CheckCircle2 className="h-5 w-5 text-green-600" />
                        )}
                        {deduction.isSeniorCitizen && (
                          <span className="px-2 py-0.5 rounded text-body-small font-medium bg-blue-100 text-blue-800">
                            Senior Citizen
                          </span>
                        )}
                      </div>
                      <div className="space-y-2">
                        ...
                        <div>
                          <span className="text-body-sm text-slate-600">Disease: </span>
                          <span className="text-body-sm font-medium text-slate-900">{deduction.disease}</span>
                        </div>
                        <div>
                          <span className="text-body-sm text-slate-600">Hospital: </span>
                          <span className="text-body-sm font-medium text-slate-900">{deduction.hospitalName}</span>
                        </div>
                        <div>
                          <span className="text-body-sm text-slate-600">Treatment Date: </span>
                          <span className="text-body-sm font-medium text-slate-900">
                        ...
                        <div className="mt-3">
                          ...
                          <div className="text-body-xs text-slate-600">Treatment Amount (Limit: ₹{deductionLimit.toLocaleString('en-IN')})</div>
                        </div>
        ...
        <div className="bg-white rounded-xl shadow-elevation-1 border border-slate-200 p-12 text-center">
          ...
          <h3 className="text-heading-md text-slate-900 mb-2">No Medical Treatments</h3>
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              ...
              <div>
                ...
                <input
                  ...
                  className={`w-full px-4 py-2 border rounded-xl focus:outline-none focus:ring-2 focus:ring-gold-500 ${
                    formErrors.patientName ? 'border-error-500' : 'border-slate-300'
                  }`}
              ...
              <div>
                ...
                <select
                  ...
                  className={`w-full px-4 py-2 border rounded-xl focus:outline-none focus:ring-2 focus:ring-gold-500 ${
                    formErrors.disease ? 'border-error-500' : 'border-slate-300'
                  }`}
                >
              ...
              <div>
                ...
                <input
                  ...
                  className={`w-full px-4 py-2 border rounded-xl focus:outline-none focus:ring-2 focus:ring-gold-500 ${
                    formErrors.treatmentDate ? 'border-error-500' : 'border-slate-300'
                  }`}
                />
              ...
              <div>
                ...
                <input
                  ...
                  className={`w-full px-4 py-2 border rounded-xl focus:outline-none focus:ring-2 focus:ring-gold-500 ${
                    formErrors.hospitalName ? 'border-error-500' : 'border-slate-300'
                  }`}
              ...
              <div>
                ...
                <input
                  ...
                  max={limit}
                  className={`w-full px-4 py-2 border rounded-xl focus:outline-none focus:ring-2 focus:ring-gold-500 ${
                    formErrors.amount ? 'border-error-500' : 'border-slate-300'
                  }`}
              ...
              <div className="flex items-start">
                ...
                />
                <label htmlFor="isSeniorCitizen" className="ml-2 block text-body-sm text-slate-700">
