...
const DeductionUtilizationDashboard = ({ filingId, formData, userAge = 30, totalIncome = 0 }) => {
  ...
  const getStatusColor = (percentage) => {
    if (percentage >= 90) return 'text-error-600 bg-error-50 border-red-200';
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        <div className="bg-gradient-to-br from-gold-50 to-gold-50 rounded-xl p-6 border border-gold-200">
          <div className="flex items-center gap-3 mb-2">
            <TrendingUp className="w-6 h-6 text-gold-600" />
            <div className="text-body-sm font-medium text-slate-700">Total Claimed</div>
          </div>
        ...
        <div className="bg-blue-50 rounded-xl p-6 border border-blue-200">
          <div className="flex items-center gap-3 mb-2">
            ...
            <div className="text-body-sm font-medium text-slate-700">Total Available</div>
          </div>
        ...
        <div className="bg-green-50 rounded-xl p-6 border border-green-200">
          <div className="flex items-center gap-3 mb-2">
            ...
            <div className="text-body-sm font-medium text-slate-700">Overall Utilization</div>
          </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
        {utilizations.sections.map((util) => {
          ...
          return (
            <div
              key={util.section}
              className={`bg-white rounded-xl border-2 p-4 ${statusColor.split(' ')[2]}`}
            >
              <div className="flex items-center justify-between mb-3">
                <div>
                  <h4 className="text-heading-sm font-semibold text-slate-900">Section {util.section}</h4>
                  <p className="text-body-xs text-slate-600 mt-1">
                    {limitConfig?.description || 'Deduction'}
                  </p>
                </div>
                {util.percentage >= 90 && (
                  <div className="px-2 py-1 text-body-small rounded bg-error-100 text-error-700 font-medium">
                    High
                  </div>
                )}
                {util.percentage === 0 && (
                  <div className="px-2 py-1 text-body-small rounded bg-slate-100 text-slate-700 font-medium">
                    Unused
                  </div>
                )}
              </div>
                ...
                {util.limit !== Infinity && (
                  <>
                    ...
                    <div className="flex justify-between text-body-xs">
                      <span className="text-slate-500">{util.percentage.toFixed(1)}% utilized</span>
                      {util.remaining > 0 && (
      ...
      {remainingOpportunities.length > 0 && (
        <div className="bg-white rounded-xl shadow-elevation-1 border border-slate-200 p-6">
          <div className="flex items-center gap-2 mb-4">
            ...
            <h3 className="text-heading-md text-slate-900">Top Deduction Opportunities</h3>
          </div>
